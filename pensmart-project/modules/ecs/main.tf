...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode([
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        awslogs-group         = "/ecs/${terraform.workspace}-${each.key}"
...
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  for_each             = var.services
  alarm_name           = "${terraform.workspace}-${each.key}-high-cpu"
  ...
  statistic            = "Average"
  threshold            = 80
  alarm_actions        = null  # No action for testing purposes, setup sns Topic and Subscription for Production
  dimensions = {
